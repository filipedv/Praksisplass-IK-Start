...
const PLAYERS = [
  { id: "p1", name: "Fatima Ali", team: "A-lag", position: "Spiss", lastLoggedDaysAgo: 0, baseWeight: 63, allergies: ["Nøtteallergi"], motherPhone: "+47 901 22 334", fatherPhone: "+47 902 33 445" },
  { id: "p2", name: "Kasper Dahl", team: "A-lag", position: "Midtbane", lastLoggedDaysAgo: 0, baseWeight: 78, allergies: [], motherPhone: "+47 903 44 556", fatherPhone: "+47 904 55 667" },
  { id: "p3", name: "Jonas Reme", team: "A-lag", position: "Forsvar", lastLoggedDaysAgo: 2, baseWeight: 82, allergies: ["Laktoseintolerant"], motherPhone: "+47 905 66 778", fatherPhone: "+47 906 77 889" },
  { id: "p4", name: "Noah Berg", team: "G19", position: "Forsvar", lastLoggedDaysAgo: 1, baseWeight: 74, allergies: [], motherPhone: "+47 907 88 990", fatherPhone: "+47 908 99 001" },
  { id: "p5", name: "Sander Lie", team: "G19", position: "Keeper", lastLoggedDaysAgo: 0, baseWeight: 76, allergies: ["Cøliaki (gluten)"], motherPhone: "+47 909 00 112", fatherPhone: "+47 910 11 223" },
  { id: "p6", name: "Magnus Solberg", team: "G19", position: "Spiss", lastLoggedDaysAgo: 5, baseWeight: 70, allergies: [], motherPhone: "+47 911 22 334", fatherPhone: "+47 912 33 445" },
  { id: "p7", name: "Emil Haugen", team: "G16", position: "Midtbane", lastLoggedDaysAgo: 3, baseWeight: 61, allergies: ["Vegetarianer"], motherPhone: "+47 913 44 556", fatherPhone: "+47 914 55 667" },
  { id: "p8", name: "Oliver Nystrøm", team: "G16", position: "Spiss", lastLoggedDaysAgo: 0, baseWeight: 58, allergies: [], motherPhone: "+47 915 66 778", fatherPhone: "+47 916 77 889" },
];
...
function PlayerDetail({ player, onBack, allergies, setAllergies, notes, setNotes }) {
  ...
  return (
    <div className="px-4 pb-4">
      ...
      {/* Emergency contact — read-only; sourced from the guardian's own
          profile, not editable by a coach. Collapsed by default; tappable
          tel: links once opened, since that's the whole point of it living
          here on mobile. */}
      <Card className="p-4 mb-4">
        <button onClick={() => setEmergencyOpen((o) => !o)} className="w-full flex items-center justify-between">
          <h3 className={cx("text-sm font-bold text-slate-900 dark:text-white flex items-center gap-1.5", isDark)}>
            <Phone size={14} className={cx("text-slate-400 dark:text-slate-500", isDark)} /> {t("emergencyContactHeading")}
          </h3>
          <ChevronDown size={15} className={cx(`text-slate-400 dark:text-slate-500 transition-transform ${emergencyOpen ? "rotate-180" : ""}`, isDark)} />
        </button>
        {emergencyOpen && (
          <div className={cx("space-y-2 mt-2.5 pt-2.5 border-t border-slate-100 dark:border-slate-800", isDark)}>
            <a href={`tel:${player.motherPhone.replace(/\s/g, "")}`} className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium text-indigo-600 flex items-center gap-1.5">
                <Phone size={12} /> {player.motherPhone}
              </span>
            </a>
            <a href={`tel:${player.fatherPhone.replace(/\s/g, "")}`} className="flex items-center justify-between">
              <span className={cx("text-sm text-slate-600 dark:text-slate-300", isDark)}>{t("fatherLabel")}</span>
              <span className="text-sm font-medium text-indigo-600 flex items-center gap-1.5">
                <Phone size={12} /> {player.fatherPhone}
              </span>
            </a>
          </div>
        )}
